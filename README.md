...
- **Copilote IA** (chat RAG) + optimisation du score d'éligibilité
- **Catalogue client** (`/client/browse`) avec filtres
- **Espace agence** : bibliothèque 3D, statistiques, paramètres

### 🔐 Transverse
- **Authentification** email/mot de passe (JWT) + **OAuth2 Google**
- **Rôles** `CLIENT` / `AGENCY` / `ADMIN` avec pages protégées
...
Après `npm run db:seed`, connectez-vous sur `/login` avec :

| Rôle | Email | Mot de passe |
| --- | --- | --- |
| **Client** | `client@viewbeforebuy.ma` | `demo1234` |
| **Agence** | `agency@viewbeforebuy.ma` | `demo1234` |
| **Admin** | `admin@viewbeforebuy.ma` | `demo1234` |
...
## 🗓️ Checklist — déploiement (fin d'été)

À faire **plus tard** (l'app tourne déjà en local sans ça) :

- [ ] Compte **Vercel** (frontend Next.js) + **Railway** ou **Render** (API + Postgres + Redis)
- [ ] Variables de production : `DATABASE_URL`, `JWT_SECRET` fort, `FRONTEND_URL`, `NEXT_PUBLIC_API_URL`
- [ ] Clés optionnelles : `OPENAI_API_KEY`, `STRIPE_SECRET_KEY`, Supabase, Google OAuth (callback URLs prod)
- [ ] `npm run db:migrate` + `npm run db:seed` sur la DB de prod
- [ ] Vérifier le badge CI vert avant de merger / déployer
